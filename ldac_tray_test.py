...
def ensure_bluetooth_active(progress_callback=None):
    ...
    try:
        res = subprocess.run(
            ["wsl", "-d", "Alpine-Test", "-u", "root", "test", "-d", "/sys/class/bluetooth/hci0"],
    ...
    if res.returncode == 0:
        ...
        chk = subprocess.run(
            ["wsl", "-d", "Alpine-Test", "-u", "root", "cat", "/sys/class/bluetooth/hci0/type"],
        ...
        if chk.returncode == 0:
            ...
            try:
                subprocess.run(
                    ["wsl", "-d", "Alpine-Test", "-u", "root", "ash", "-c", startup_cmd],
    ...
    try:
        subprocess.run(["wsl", "-d", "Alpine-Test", "-u", "root", "modprobe", "vhci-hcd"], creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=10)
    except subprocess.TimeoutExpired:
        pass
    try:
        subprocess.run(["wsl", "-d", "Alpine-Test", "-u", "root", "modprobe", "btusb"], creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=10)
    except subprocess.TimeoutExpired:
        pass
    time.sleep(1)

    # Mantener viva la VM durante todo el attach + espera de hci0 (30s de margen)
    boot_proc = subprocess.Popen(["wsl", "-d", "Alpine-Test", "-u", "root", "sleep", "30"], creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo())
    time.sleep(1)

    log("Acoplando adaptador Bluetooth a WSL2...")
    try:
        subprocess.run([USBIPD, "attach", "--wsl", "Alpine-Test", "--busid", BUSID], creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=20)
    except subprocess.TimeoutExpired:
...
def run_scan_bg(status_var, listbox, btn_scan, btn_connect):
    try:
        ...
        try:
            subprocess.run(
                ["wsl", "-d", "Alpine-Test", "-u", "root", "ash", "-c",
...
def run_connect_bg(selected_device_str, status_var, btn_scan, btn_connect, win, lbl_current):
    ...
    def get_device_info(target_mac):
        try:
            chk = subprocess.run(
                ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", f"echo 'info {target_mac}' | bluetoothctl"],
                capture_output=True, text=True,
    ...
    try:
        ...
        res_conn = subprocess.run(
            ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", "echo 'devices Connected' | bluetoothctl"],
            capture_output=True, text=True,
        ...
        if connected_macs:
            for c_mac in connected_macs:
                if c_mac.upper() != mac.upper():
                    ...
                    subprocess.run(
                        ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", f"(sleep 1.2; echo 'disconnect {c_mac}'; sleep 3) | bluetoothctl"],
                        creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=10
                    )
        ...
        if not paired:
            ...
            subprocess.run(
                ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", f"(sleep 1.2; echo 'pair {mac}'; sleep 8) | bluetoothctl"],
        ...
        if not trusted:
            ...
            subprocess.run(
                ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", f"(sleep 1.2; echo 'trust {mac}'; sleep 1) | bluetoothctl"],
                creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=8
            )
            time.sleep(0.5)
        ...
        subprocess.run(
            ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", f"(sleep 1.2; echo 'connect {mac}'; sleep 8) | bluetoothctl"],
            creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=15
        )
    ...
    def on_clear_devices():
        ...
        def run_clear_bg():
            try:
                ...
                res = subprocess.run(
                    ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", "echo 'devices' | bluetoothctl"],
                    capture_output=True, text=True,
                    creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=10
                )
                ...
                if macs:
                    for target_mac in macs:
                        ...
                        subprocess.run(
                            ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", f"echo 'remove {target_mac}' | bluetoothctl"],
                            creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo(), timeout=6
                        )
...
def start_ldac():
    ...
    try:
        ...
        run(["wsl", "-d", "Alpine-Test", "-u", "root", "modprobe", "vhci-hcd"])
        run(["wsl", "-d", "Alpine-Test", "-u", "root", "modprobe", "btusb"])
        ...
        if os.path.exists(receptor_local):
            try:
                ...
                proc = subprocess.Popen(
                    ["wsl", "-d", "Alpine-Test", "-u", "root", "sh", "-c", "cat > /root/receptor_audio.sh"],
                ...
                pass
        run(["wsl", "-d", "Alpine-Test", "-u", "root", "chmod", "+x", "/root/receptor_audio.sh"])
        time.sleep(1)

        # Mantener viva la distribución durante el attach
        boot_proc = subprocess.Popen(["wsl", "-d", "Alpine-Test", "-u", "root", "sleep", "10"], creationflags=CREATE_NO_WINDOW, startupinfo=_startupinfo())
        time.sleep(1)

        # 3. Attach USBIPD
        run([USBIPD, "attach", "--wsl", "Alpine-Test", "--busid", BUSID])
        ...
        for _ in range(15):
            ...
            result = subprocess.run(
                ["wsl", "-d", "Alpine-Test", "-u", "root", "test", "-d",
...
def stop_ldac_cleanup():
    ...
    if not keep_wsl_alive:
        ...
        run(["wsl", "-d", "Alpine-Test", "-u", "root", "killall", "-9", "nc", "pacat"])
